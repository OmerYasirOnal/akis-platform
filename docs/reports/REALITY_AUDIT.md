...
| **Port 80/443 conflict on single-server deployment** | Both staging and prod Caddy tried to bind 80/443; nginx also running on server | Deploy failed with "address already in use" error | **FIXED:** Implemented single edge proxy pattern - only one service binds 80/443 | ✅ Completed |
