...
const docPackFileSchema = z.object({
  path: z.string().min(1),
  summary: z.string().min(1),
  constraints: z.array(z.string().min(1)).default([]),
});

export const DocPackFromRepoInputSchema = z.object({
  repository: repositorySchema,
  objective: z.string().min(1).optional(),
  files: z.array(docPackFileSchema).min(1).max(50),
});

export const DocPackFromRepoOutputSchema = z.object({
  skill: z.literal('DocPackFromRepo'),
  version: z.literal('1.0'),
  sections: z.object({
    title: z.string().min(1),
    summary: z.string().min(1),
    files: z.array(docPackFileSchema),
...
const releasePrSchema = z.object({
...
export const ReleaseNotesFromPRsInputSchema = z.object({
  releaseVersion: z.string().min(1),
  pullRequests: z.array(releasePrSchema).min(1).max(200),
  breakingChanges: z.array(z.string().min(1)).default([]),
});

const releaseNoteChangeSchema = z.object({
  number: z.number().int().positive(),
  title: z.string().min(1),
  type: z.enum(['feature', 'fix', 'maintenance', 'breaking']),
});

export const ReleaseNotesFromPRsOutputSchema = z.object({
  skill: z.literal('ReleaseNotesFromPRs'),
  version: z.literal('1.0'),
  sections: z.object({
    releaseVersion: z.string().min(1),
    highlights: z.array(z.string()),
    changes: z.array(releaseNoteChangeSchema),
...
export const ChecklistFromRunbookInputSchema = z.object({
  runbookTitle: z.string().min(1),
  steps: z.array(runbookStepSchema).min(1).max(100),
});

const checklistItemSchema = runbookStepSchema.extend({
  id: z.string().regex(/^CHK-\d{2}$/),
});

export const ChecklistFromRunbookOutputSchema = z.object({
  skill: z.literal('ChecklistFromRunbook'),
  version: z.literal('1.0'),
  sections: z.object({
    runbookTitle: z.string().min(1),
...
export function runScribeSkill(
  skill: ScribeSkillName,
  input: unknown
): z.infer<typeof DocPackFromRepoOutputSchema> | z.infer<typeof ReleaseNotesFromPRsOutputSchema> | z.infer<typeof ChecklistFromRunbookOutputSchema>;
export function runScribeSkill(skill: ScribeSkillName, input: unknown): unknown {
  if (skill === 'DocPackFromRepo') {
    ...
    const output = {
      ...
      markdown: [
        `# DocPack: ${parsed.repository.owner}/${parsed.repository.repo}`,
        '',
        '## Summary',
        parsed.objective ?? `Generated from ${files.length} repository files.`,
        '',
        '## Files',
        ...files.map((file) => `- ${file.path}: ${file.summary}`),
      ].join('\n'),
    };
    return DocPackFromRepoOutputSchema.parse(output);
  }

  if (skill === 'ReleaseNotesFromPRs') {
    const parsed = ReleaseNotesFromPRsInputSchema.parse(input);
    const changes = [...parsed.pullRequests]
      .sort((a, b) => a.number - b.number)
      .map((pr) => ({ number: pr.number, title: pr.title, type: toType(pr.labels) }));
    ...
    const output = {
      ...
      markdown: [
        `# Release ${parsed.releaseVersion}`,
        '',
        '## Highlights',
        ...(highlights.length > 0 ? highlights.map((item) => `- ${item}`) : ['- None']),
        '',
        '## Changes',
        ...changes.map((item) => `- [${item.type}] #${item.number} ${item.title}`),
      ].join('\n'),
    };
    return ReleaseNotesFromPRsOutputSchema.parse(output);
  }

  const parsed = ChecklistFromRunbookInputSchema.parse(input);
  const checklist = parsed.steps.map((step, index) => ({ id: `CHK-${String(index + 1).padStart(2, '0')}`, ...step }));
  const output = {
    skill: 'ChecklistFromRunbook' as const,
    version: '1.0' as const,
    sections: {
      runbookTitle: parsed.runbookTitle,
      checklist,
      mandatoryCount: checklist.filter((item) => item.mandatory).length,
    },
    markdown: [
      `# ${parsed.runbookTitle}`,
      '',
      '## Checklist',
      ...checklist.map((item) => `- [ ] ${item.id} ${item.title} | Action: ${item.action} | Verify: ${item.verification}`),
    ].join('\n'),
  };
  return ChecklistFromRunbookOutputSchema.parse(output);
}
